...
const EmbeddingContextLengthSelect = () => {
  const [value, setValue] = useState(2048);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    const stored = localStorage.getItem('embeddingContextLength');
  ...
  const handleSave = async (newValue: number) => {
    ...
    setValue(newValue);
    try {
      localStorage.setItem('embeddingContextLength', String(newValue));
      window.dispatchEvent(new Event('client-config-changed'));
      await fetch('/api/settings', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ embeddingContextLength: String(newValue) }),
      });
    } catch (error) {
      console.error('Error saving embedding context length:', error);
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] lg:text-xs text-black/50 dark:text-white/50">
              Max tokens for embedding model input. Controls chunk size for
              long texts. Default 2K.
            </p>
          </div>
...
export default EmbeddingContextLengthSelect;